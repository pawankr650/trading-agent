...
WEB = ROOT / "web" / "static"  # the one-page terminal UI (plain HTML/CSS/JS, no build step)
...
async def lifespan(_app):
    ...
    if os.getenv("STOCKPILOT_NO_ENGINE") != "1":  # tests switch the live news pipeline off
...
api = Depends(auth)
app.include_router(terminal.router, dependencies=[api])
